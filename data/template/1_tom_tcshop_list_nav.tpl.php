...
var list_cate_child_id      = "<?php echo $cate_child_id;?>";
...
function open_list(){
    pageIndex = 1;
    var url = 'plugin.php?id=tom_tcshop&site=<?php echo $site_id;?>&mod=list'+'&city_id='+list_city_id+'&area_id='+list_area_id+'&street_id='+list_street_id+'&cate_id='+list_cate_id+'&cate_child_id='+list_cate_child_id+'&keyword='+list_keyword+'&ordertype='+list_ordertype;
    window.location = url;
}
...
$(".id-catediv").on("click", ".item", function (event) {
    ...
    $.ajax({
        type: "GET",
        url: "<?php echo $ajaxGetCateChildUrl;?>",
        data: { cate_id:cateId},
        success: function(msg){
            var data = eval('('+msg+')');
            if(data == 100){

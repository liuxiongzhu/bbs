...
if($_GET['act'] == 'ok1'){
    ...
    $tixianInfo = C::t('#tom_tcshop#tom_tcshop_money_tixian')->fetch_by_id($_GET['id']);
    ...
    if(empty($tixianInfo['tx_order_no'])){
        ...
        C::t('#tom_tcshop#tom_tcshop_money_tixian')->update($tixianInfo['id'],$updateData);
    ...
    if(is_array($_GET['ids']) && !empty($_GET['ids'])){
        foreach ($_GET['ids'] as $key => $value){
            ...
            $tixianInfo = C::t('#tom_tcshop#tom_tcshop_money_tixian')->fetch_by_id($value);
    ...
    $count = C::t('#tom_tcshop#tom_tcshop_money_tixian')->fetch_all_count(" {$where} ");
    $tixianList = C::t('#tom_tcshop#tom_tcshop_money_tixian')->fetch_all_list(" {$where} "," ORDER BY id DESC ",$start,$pagesize);
    ...
    foreach ($tixianList as $key => $value) {
        ...
        echo '<td>' .$userInfo['nickname'] . '(UID:'.$userInfo['id'].')'.'<a target="_blank" href="'.$adminBaseUrl.'&tmod=money&act=log&user_id='.$userInfo['id'].'&formhash='.FORMHASH.'">(' . $Lang['moneylog_list_title']. ')</a>'.'</td>';

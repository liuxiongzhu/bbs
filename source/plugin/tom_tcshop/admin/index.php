...
SCRIPT;
    ...
    $count = C::t('#tom_tcshop#tom_tcshop_photo')->fetch_all_count(" AND tcshop_id={$tcshop_id} AND type_id=1 ");
    $photoList = C::t('#tom_tcshop#tom_tcshop_photo')->fetch_all_list(" AND tcshop_id={$tcshop_id} AND type_id=1 ","ORDER BY paixu ASC,id ASC",$start,$pagesize);
    __create_nav_html();
    ...
    echo '<form name="cpform2" id="cpform2" method="post" autocomplete="off" action="'.ADMINSCRIPT.'?action='.$modFromUrl.'&act=photopaixu&tcshop_id='.$tcshop_id.'&formhash='.FORMHASH.'">'.
		'<input type="hidden" name="formhash" value="'.FORMHASH.'" />';
    showtableheader();
    echo '<tr class="header">';
    echo '<th>' . $Lang['photo_paixu'] . '</th>';
    ...
    foreach ($photoList as $key => $value) {
        echo '<tr>';
        echo '<td><input type="hidden" name="ids[]" value="' . $value['id'] . '" ><input name="paixus[]" type="text" value="'.$value['paixu'].'" size="5" /></td>';
    ...
    $formstr = <<<EOF
        <tr>
            <td colspan="15">
                <div class="fixsel"><input type="submit" class="btn" id="submit_announcesubmit" name="announcesubmit" value="{$Lang['photo_paixu_btn']}" /></div>
            </td>
        </tr>
EOF;
    echo $formstr;
    ...
    if(is_array($_GET['ids']) && !empty($_GET['ids'])){
        foreach ($_GET['ids'] as $key => $value){
            $updateData = array();
            $updateData['paixu']        = $_GET['paixus'][$key];
            C::t('#tom_tcshop#tom_tcshop_photo')->update($value,$updateData);
        }
    }
    ...
    if(submitcheck('submit')){
        ...
        $insertData['tcshop_id']    = $tcshop_id;
        $insertData['picurl']       = $imageName;
        $insertData['thumb']        = $imageName;
        $insertData['add_time']     = TIMESTAMP;
        C::t('#tom_tcshop#tom_tcshop_photo')->insert($insertData);
        
        cpmsg($Lang['act_success'], $adminListUrl.'&tmod=index'."&act=focuspic&tcshop_id=".$_GET['tcshop_id'], 'succeed');
    ...
    $page = intval($_GET['page'])>0? intval($_GET['page']):1;
    $start = ($page-1)*$pagesize;
    $count = C::t('#tom_tcshop#tom_tcshop_photo')->fetch_all_count(" AND tcshop_id={$tcshop_id} AND type_id=2 ");
    $photoList = C::t('#tom_tcshop#tom_tcshop_photo')->fetch_all_list(" AND tcshop_id={$tcshop_id} AND type_id=2 ","ORDER BY paixu ASC,id ASC",$start,$pagesize);
    __create_nav_html();
    showformheader($modFromUrl.'&act=focuspic&tcshop_id='.$tcshop_id,'enctype');
    showtableheader();
    tomshowsetting(true,array('title'=>$Lang['photo_picurl'],'name'=>'picurl','value'=>$options['picurl'],'msg'=>$Lang['photo_picurl_msg']),"file");
    showsubmit('submit', 'submit');
    showtablefooter();
    ...
    $modBasePageUrl = $adminBaseUrl.'&tmod=index&act=focuspic&tcshop_id='.$tcshop_id.'&formhash='.FORMHASH;
    
    echo '<form name="cpform2" id="cpform2" method="post" autocomplete="off" action="'.ADMINSCRIPT.'?action='.$modFromUrl.'&act=focuspicpaixu&tcshop_id='.$tcshop_id.'&formhash='.FORMHASH.'">'.
		'<input type="hidden" name="formhash" value="'.FORMHASH.'" />';
    showtableheader();
    echo '<tr class="header">';
    echo '<th>' . $Lang['photo_paixu'] . '</th>';
    ...
    foreach ($photoList as $key => $value) {
        ...
        echo '<td>';
        echo '<a href="'.$modBaseUrl.'&act=delfocuspic&id='.$value['id'].'&tcshop_id='.$tcshop_id.'&formhash='.FORMHASH.'">' . $Lang['delete'] . '</a>';
        echo '</td>';
        echo '</tr>';
    }
    showtablefooter();
...
EOF;
    echo $formstr;
    showformfooter();
}else if($_GET['formhash'] == FORMHASH && $_GET['act'] == 'focuspicpaixu'){
...
}else if($_GET['formhash'] == FORMHASH && $_GET['act'] == 'delfocuspic'){
    
    C::t('#tom_tcshop#tom_tcshop_photo')->delete_by_id($_GET['id']);
    
    cpmsg($Lang['act_success'], $adminListUrl.'&tmod=index'."&act=focuspic&tcshop_id=".$_GET['tcshop_id'], 'succeed');
    
}else if($_GET['act'] == 'proof'){
    ...
    if(submitcheck('submit')){
        ...
        cpmsg($Lang['act_success'], $adminListUrl.'&tmod=index'."&act=proof&tcshop_id=".$_GET['tcshop_id'], 'succeed');
    ...
    $page = intval($_GET['page'])>0? intval($_GET['page']):1;
    $start = ($page-1)*$pagesize;
    $count = C::t('#tom_tcshop#tom_tcshop_photo')->fetch_all_count(" AND tcshop_id={$tcshop_id} AND type_id=3 ");
    $photoList = C::t('#tom_tcshop#tom_tcshop_photo')->fetch_all_list(" AND tcshop_id={$tcshop_id} AND type_id=3 ","ORDER BY paixu ASC,id ASC",$start,$pagesize);
    __create_nav_html();
    showformheader($modFromUrl.'&act=proof&tcshop_id='.$tcshop_id,'enctype');
    showtableheader();
    tomshowsetting(true,array('title'=>$Lang['photo_picurl'],'name'=>'picurl','value'=>$options['picurl'],'msg'=>$Lang['photo_picurl_msg']),"file");
    showsubmit('submit', 'submit');
    showtablefooter();
    ...
    $modBasePageUrl = $adminBaseUrl.'&tmod=index&act=proof&tcshop_id='.$tcshop_id.'&formhash='.FORMHASH;
    ...
    foreach ($photoList as $key => $value) {
        echo '<tr>';
        echo '<td><a href="'. tomgetfileurl($value['picurl']) .'" target="_blank"><img src="' . tomgetfileurl($value['picurl']) . '" width="60" /></a></td>';
        if($value['type_id'] == 1){
            echo '<td>'. $Lang['photo_type_1'] .'</td>';
        }else if($value['type_id'] == 2){
            echo '<td>'. $Lang['photo_type_2'] .'</td>';
        }else if($value['type_id'] == 3){
            echo '<td>'. $Lang['photo_type_3'] .'</td>';
        }
        echo '<td>';
        echo '<a href="'.$modBaseUrl.'&act=delproof&id='.$value['id'].'&tcshop_id='.$tcshop_id.'&formhash='.FORMHASH.'">' . $Lang['delete'] . '</a>';
        echo '</td>';
        echo '</tr>';
    }
    showtablefooter();
...
}else if($_GET['formhash'] == FORMHASH && $_GET['act'] == 'delproof'){
    
    C::t('#tom_tcshop#tom_tcshop_photo')->delete_by_tcshop_id_proof($_GET['id']);
    ...
    if(submitcheck('submit')){
        ...
        if($vip_time < TIMESTAMP){
            ...
            if($tcshopConfig['base_time_type'] == 1 && $tcshopConfig['open_ruzhu_one'] == 1 ){
...
}else if($_GET['formhash'] == FORMHASH && $_GET['act'] == 'batch_qunfa'){
    
    $idsstr = implode(',', $_GET['ids']);
    
    showformheader($modFromUrl.'&act=batch_qunfa_do&formhash='.FORMHASH);
    showtableheader();
    echo '<tr><th colspan="15" class="partition">' . $Lang['qunfa_send_title'] . '</th></tr>';
    tomshowsetting(true,array('title'=>$Lang['qunfa_send_text'],'name'=>'content','value'=>'','msg'=>$Lang['qunfa_send_text_msg']),"textarea");
    $link_item = array('xufei'=>$Lang['qunfa_send_link_xufei'],'score'=>$Lang['qunfa_send_link_score'],'zdy'=>$Lang['qunfa_send_link_zdy']);
    tomshowsetting(true,array('title'=>$Lang['qunfa_send_link'],'name'=>'link','value'=>'xufei','msg'=>$Lang['qunfa_send_link_msg'],'item'=>$link_item),"radio");
    tomshowsetting(true,array('title'=>$Lang['qunfa_send_zdylink'],'name'=>'zdylink','value'=>'','msg'=>$Lang['qunfa_send_zdylink_msg']),"input");
    echo '<input type="hidden" name="idsstr" value="'.$idsstr.'" />';
    showsubmit('submit', 'submit');
    showtablefooter();
    showformfooter();
    
}else if($_GET['formhash'] == FORMHASH && $_GET['act'] == 'batch_qunfa_do'){
    
    $content    = isset($_GET['content'])? addslashes($_GET['content']):'';
    $link       = isset($_GET['link'])? addslashes($_GET['link']):'';
    $zdylink    = isset($_GET['zdylink'])? addslashes($_GET['zdylink']):'';
    $idsstr     = isset($_GET['idsstr'])? addslashes($_GET['idsstr']):'';
    ...
    if(is_array($ids) && !empty($ids)){
        foreach ($ids as $key => $value){
            ...
            $tcshopInfo = C::t('#tom_tcshop#tom_tcshop')->fetch_by_id($value);
            $tcUserInfo = C::t('#tom_tongcheng#tom_tongcheng_user')->fetch_by_id($tcshopInfo['user_id']);
            
            if(in_array($tcshopInfo['user_id'], $do_user_ids)){
                continue;
            }else{
                $do_user_ids[] = $tcshopInfo['user_id'];
            }
            
            $url = $_G['siteurl']."plugin.php?id=tom_tcshop&site={$tcshopInfo['site_id']}&mod=index";
            if($link == 'zdy'){
                $url = $zdylink;
            }else if($link == 'xufei'){
                $url = $_G['siteurl']."plugin.php?id=tom_tcshop&site={$tcshopInfo['site_id']}&mod=mylist";
            }else if($link == 'score'){
                $url = $_G['siteurl']."plugin.php?id=tom_tcshop&site={$tcshopInfo['site_id']}&mod=store";
            }
            ...
            $access_token = $weixinClass->get_access_token();
            if($access_token && !empty($tcUserInfo['openid'])){
                $templateSmsClass = new templateSms($access_token, $url);
                $smsData = array(
                    'first'         => $content,
                    'keyword1'      => $tcshopConfig['plugin_name'],
                    'keyword2'      => dgmdate(TIMESTAMP,"Y-m-d H:i:s",$tomSysOffset),
            ...
            $insertData['user_id']      = $tcUserInfo['id'];
            $insertData['type']         = 1;
            $insertData['content']      = '<font color="#238206">'.$tcshopConfig['plugin_name'].'</font><br/>'.$content.'<br/><a href="'.$url.'">['.$Lang['qunfa_tz_btn'].']</a>';
            $insertData['is_read']      = 0;
            $insertData['tz_time']      = TIMESTAMP;
            C::t('#tom_tongcheng#tom_tongcheng_tz')->insert($insertData);
        }
...
.tc_content_box_handle {    width: 180px;}
.tc_content_box_handle li{ list-style-type: none; height: 28px; line-height: 25px;width: 90px;
...
EOF;
    ...
    if($over_status > 0){
        ...
        if($over_status == 2){
            $where.= " AND status IN (1,2) AND vip_level=0 AND  base_level=2 AND base_time<{$over_time} ";
        }
        if($over_status == 3){
            $where.= " AND vip_level=1 AND vip_time<{$over_time} ";
        }
    }
    ...
    $modBasePageUrl = $modBaseUrl."&site_id={$site_id}&user_id={$user_id}&shenhe_status={$shenhe_status}&over_status={$over_status}";
    ...
    $over_status_1 = $over_status_2 = $over_status_3 = $over_status_4 = "";
    if($over_status == 1){ $over_status_1 = "selected";}
    if($over_status == 2){ $over_status_2 = "selected";}
    if($over_status == 3){ $over_status_3 = "selected";}
    $overStr = '<tr><td width="100" align="right"><b>'.$Lang['index_search_over_status'].'</b></td>';
    $overStr.= '<td><select style="width: 260px;" name="over_status" id="over_status">';
    $overStr.=  '<option value="0">'.$Lang['index_search_over_status_0'].'</option>';
    $overStr.=  '<option value="1" '.$over_status_1.'>'.$Lang['index_search_over_status_1'].'</option>';
    $overStr.=  '<option value="2" '.$over_status_2.'>'.$Lang['index_search_over_status_2'].'</option>';
    $overStr.=  '<option value="3" '.$over_status_3.'>'.$Lang['index_search_over_status_3'].'</option>';
    ...
    foreach ($tcshopList as $key => $value) {
        ...
        if(!empty($value['business_licence'])){
            if(!preg_match('/^http/', $value['business_licence']) ){
                if(strpos($value['business_licence'], 'source/plugin/tom_tcshop/') === FALSE){
            ...
            $business_licence = 'source/plugin/tom_tcshop/images/business_licence.png';
        }
        ...
        echo '<td>';
        echo '<a href="'.$business_licence.'" target="_blank"><img src="'.$business_licence.'" width="40" height="40"  /></a><br/>';
        if(is_array($proofList) && !empty($proofList)){
            foreach($proofList as $pk => $pv){
                echo '<a href="'.$pv['picurl'].'" target="_blank"><img src="'.$pv['picurl'].'" width="40" height="40" style="margin:5px 5px 0 5px;" /></a>';
            }
        }
        echo '</td>';
        ...
        echo '<li><a href="'.$modBaseUrl.'&act=focuspic&tcshop_id='.$value['id'].'&formhash='.FORMHASH.'">' . $Lang['tcshop_focuspic']. '</a></li>';
        if($cateInfoTmp['open_upload_proof'] == 1 || count($proofList) > 0){
            echo '<li><a href="'.$modBaseUrl.'&act=proof&tcshop_id='.$value['id'].'&formhash='.FORMHASH.'">' . $Lang['tcshop_proof']. '</a></li>';
        }
        ...
        echo '<li><a href="'.$adminBaseUrl.'&tmod=mallCate&tcshop_id='.$value['id'].'&formhash='.FORMHASH.'">' . $Lang['mallCate_list_title']. '</a></li>';
        if($value['status'] == 1 ){
...
EOF;
    ...
    $formstr = <<<EOF
        <tr>
            ...
            <td class="td25">
                <select name="act" >
                    <option value="batch_qunfa">{$Lang['batch_qunfa']}</option>
                </select>
            </td>
            <td colspan="15">
        ...
        <script type="text/javascript">
        function batch_do(){
          var r = confirm("{$Lang['makesure_do_msg']}")
          if (r == true){
            return true;
          }else{
            return false;
...
function __create_info_html($infoArr = array()){
    ...
    tomshowsetting(true,array('title'=>$Lang['tcshop_tabs'],'name'=>'tabs','value'=>$options['tabs'],'msg'=>$Lang['tcshop_tabs_msg']),"input");
    ...
    tomshowsetting(true,array('title'=>$Lang['tcshop_vr_url'],'name'=>'vr_url','value'=>$options['vr_url'],'msg'=>$Lang['tcshop_vr_url_msg']),"input");
    ...
    tomshowsetting(true,array('title'=>$Lang['tcshop_clicks'],'name'=>'clicks','value'=>$options['clicks'],'msg'=>''),"input");
...
function __create_nav_html($infoArr = array()){
    ...
    if($_GET['act'] == 'add'){
        ...
        tomshownavli($Lang['tcshop_list_title'],$modBaseUrl,false);
        tomshownavli($Lang['tcshop_proof'],"",true);
    }else if($_GET['act'] == 'focuspic'){
        tomshownavli($Lang['tcshop_list_title'],$modBaseUrl,false);
        tomshownavli($Lang['tcshop_focuspic'],"",true);
    }else{
        tomshownavli($Lang['tcshop_list_title'],$modBaseUrl,false);
        tomshownavli($Lang['tcshop_add'],$modBaseUrl."&act=add",false);
        if($shenhe_status == 2){

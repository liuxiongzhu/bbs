...
if($_GET['act'] == 'add'){
    if(submitcheck('submit')){
        ...
        C::t('#tom_tcshop#tom_tcshop_focuspic')->insert($insertData);
    ...
    $focuspicInfo = C::t('#tom_tcshop#tom_tcshop_focuspic')->fetch_by_id($_GET['id']);
    if(submitcheck('submit')){
        $updateData = array();
        $updateData = __get_post_data($focuspicInfo);
        C::t('#tom_tcshop#tom_tcshop_focuspic')->update($focuspicInfo['id'],$updateData);
        cpmsg($Lang['act_success'], $modListUrl, 'succeed');
    }else{
        tomloadcalendarjs();
        loadeditorjs();
    ...
    C::t('#tom_tcshop#tom_tcshop_focuspic')->delete_by_id($_GET['id']);
    cpmsg($Lang['act_success'], $modListUrl, 'succeed');
}else{
    ...
    $focuspicList = C::t('#tom_tcshop#tom_tcshop_focuspic')->fetch_all_list("{$where}"," ORDER BY fsort ASC,id DESC ",$start,$pagesize);

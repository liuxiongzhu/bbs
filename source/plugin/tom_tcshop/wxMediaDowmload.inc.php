...
if($_GET['act'] == "photo" && $_GET['formhash'] == $formhash){
    ...
    if(isset($types[$return['info']['content_type']])){
        ...
        $imageDir = "/source/plugin/tom_tcshop/data/photo/".date("Ym")."/".date("d")."/";
        $imageName = "source/plugin/tom_tcshop/data/photo/".date("Ym")."/".date("d")."/".md5($media_id).$types[$return['info']['content_type']];
        $thumbName = "source/plugin/tom_tcshop/data/photo/".date("Ym")."/".date("d")."/".md5($media_id).'_thumb'.$types[$return['info']['content_type']];
        ...
        if(false !== file_put_contents(DISCUZ_ROOT.'./'.$imageName, $return['content'])){
            if(false !== file_put_contents(DISCUZ_ROOT.'./'.$thumbName, $return['content'])){
                require_once libfile('class/image');
                $image = new image();
                $image->Thumb(DISCUZ_ROOT.'./'.$imageName,'', 720, 720, 1, 1);
                $image->Thumb(DISCUZ_ROOT.'./'.$thumbName,'', 720, 324, 2, 1);
                $outAtr = array(
                    'status' => 200,
                    'picurl' => $imageName,
                    'thumburl' => $thumbName,

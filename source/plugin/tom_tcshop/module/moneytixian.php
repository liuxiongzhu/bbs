...
if($_GET['act'] == 'save' && $_GET['formhash'] == FORMHASH){
    ...
    if($__UserInfo['shop_money'] < $money){
        $outArr = array(
            'status'=> 303,
        );
        echo json_encode($outArr); exit;
    ...
    if(C::t('#tom_tcshop#tom_tcshop_money_tixian')->insert($insertData)){
        
        $tixianId = C::t('#tom_tcshop#tom_tcshop_money_tixian')->insert_id();
        
        $updateData = array();
        $updateData['shop_money']            = $__UserInfo['shop_money'] - $money;
        $updateData['shop_tixian_money']     = $__UserInfo['shop_tixian_money'] + $money;
        ...
        $insertData['old_money']    = $__UserInfo['shop_money'];
        $insertData['business_user_id']= $__UserInfo['id'];
        $insertData['title']        = lang("plugin/tom_tcshop", "tixian_title");
        ...
        if($access_token && !empty($toUser['openid'])){
            ...
            $template_first = str_replace("{NAME}",$__UserInfo['nickname'], lang('plugin/tom_tcshop','tixian_template_first'));
...
$saveUrl = "plugin.php?id=tom_tcshop&site={$site_id}&mod=moneytixian&act=save";


$isGbk = false;
if (CHARSET == 'gbk') $isGbk = true;
include template("tom_tcshop:moneytixian");
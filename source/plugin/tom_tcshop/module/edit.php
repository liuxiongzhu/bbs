...
if($_GET['act'] == 'save' && $_GET['formhash'] == FORMHASH){
    ...
    $proofArr = array();
    if(is_array($_GET['proof']) && !empty($_GET['proof'])){
        foreach($_GET['proof'] as $key => $value){
            if(!empty($value)){
                $proofArr[] = addslashes($value);
            }
        }
    }
    ...
    if(C::t('#tom_tcshop#tom_tcshop')->update($tcshop_id,$updateData)){
        ...
        if(($tcshopInfo['shenhe_status'] == 2 && $tcshopInfo['is_ok'] == 0) || $tcshopInfo['shenhe_status'] == 3){
            ...
            if($access_token && !empty($toUser['openid'])){
                ...
                $shenhe_template_first = lang('plugin/tom_tcshop','shenhe_template_first');
                $shenhe_template_first = str_replace("{SHOPNAME}",$tcshopInfo['name'],$shenhe_template_first);
                $smsData = array(
...
$tcshopPhotoListTmp = C::t('#tom_tcshop#tom_tcshop_photo')->fetch_all_list(" AND tcshop_id={$tcshopInfo['id']} AND type_id = 1 "," ORDER BY paixu ASC,id ASC ",0,50);
...
if(is_array($tcshopPhotoListTmp) && !empty($tcshopPhotoListTmp)){
    foreach ($tcshopPhotoListTmp as $kk => $vv){
        ...
        $tcshopPhotoList[$kk]['paixu'] = $vv['paixu'];
...
$notEditProof = 0;
if($tcshopConfig['open_not_edit_proof'] == 1 && $proofCount > 0 && $tcshopInfo['shenhe_status'] == 1){
    $notEditProof = 1;
}
...
$uploadUrl5 = "plugin.php?id=tom_tcshop&site={$site_id}&mod=upload&act=proofphoto&formhash=".FORMHASH;

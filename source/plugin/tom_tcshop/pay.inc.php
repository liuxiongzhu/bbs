...
if($site_id > 1){
    ...
    if($sitesInfoTmp){
        if($sitesInfoTmp['ruzhu_price'] > 0){
            $tcshopConfig['ruzhu_price'] = $sitesInfoTmp['ruzhu_price'];
        }
        if($sitesInfoTmp['vip_ruzhu_price'] > 0){
            $tcshopConfig['vip_ruzhu_price'] = $sitesInfoTmp['vip_ruzhu_price'];
        }
        if($sitesInfoTmp['vip_ruzhu_price_two'] > 0){
            $tcshopConfig['vip_ruzhu_price_two'] = $sitesInfoTmp['vip_ruzhu_price_two'];
        }
    }
}
...
if($act == "ruzhu" && $_GET['formhash'] == FORMHASH){
    ...
    $photoArr = $photoThumbArr = $photoSortArr = array();
    ...
    $tcshopListTmp = C::t('#tom_tcshop#tom_tcshop')->fetch_all_list(" AND user_id={$user_id} ", " ORDER BY id DESC ", 0, 1);
    ...
    if(C::t('#tom_tcshop#tom_tcshop')->insert($insertData)){
        ...
        if(is_array($photoArr) && !empty($photoArr)){
            foreach ($photoArr as $key => $value){
                ...
                $insertData['paixu']     = $photoSortArr[$key];
    ...
    $noPayOrderListTmp = C::t('#tom_tongcheng#tom_tongcheng_order')->fetch_all_list(" AND tcshop_id={$tcshop_id} AND user_id={$userInfo['id']} AND order_type=4 AND order_status=1 ","ORDER BY id DESC",0,1);
    if(is_array($noPayOrderListTmp) && !empty($noPayOrderListTmp[0]['pay_price']) && $noPayOrderListTmp[0]['pay_price']>0){
        $pay_price = $noPayOrderListTmp[0]['pay_price'];
    }
...
}else if($act == "xufeiVip" && $_GET['formhash'] == FORMHASH){
    ...
    if(!empty($tcshopConfig['vip_xufei_price'])){
        $pay_price = $tcshopConfig['vip_xufei_price'];
    }else{
        $pay_price = $tcshopConfig['vip_ruzhu_price'];
    }
    
    $orderListTmp = C::t('#tom_tongcheng#tom_tongcheng_order')->fetch_all_list(" AND tcshop_id={$tcshop_id} AND user_id={$userInfo['id']} AND order_type=8 AND order_status=1 ","ORDER BY id DESC",0,10);
    if(is_array($orderListTmp) && !empty($orderListTmp)){
        foreach ($orderListTmp as $key => $value){
            $updateData = array();
            $updateData['order_status'] = 3;
    ...
    $orderListTmp = C::t('#tom_tongcheng#tom_tongcheng_order')->fetch_all_list(" AND tcshop_id={$tcshop_id} AND user_id={$userInfo['id']} AND order_type=9 AND order_status=1 ","ORDER BY id DESC",0,10);
    if(is_array($orderListTmp) && !empty($orderListTmp)){
        foreach ($orderListTmp as $key => $value){
            $updateData = array();
            $updateData['order_status'] = 3;
    ...
    if($tcshopConfig['tel_price'] > 0){
        ...
        $order_no = "TC".date("YmdHis")."-".mt_rand(111111, 666666);
        $pay_price = $tcshopConfig['tel_price'];
        ...
        if(C::t('#tom_tongcheng#tom_tongcheng_order')->insert($insertData)){
            ...
            $insertData['goods_url']       = "plugin.php?id=tom_tcshop&site={$site_id}&mod=details&tcshop_id={$tcshop_id}";
            $insertData['succ_back_url']   = "plugin.php?id=tom_tcshop&site={$site_id}&mod=details&tcshop_id={$tcshop_id}";
            $insertData['fail_back_url']   = "plugin.php?id=tom_tcshop&site={$site_id}&mod=details&tcshop_id={$tcshop_id}";
            $insertData['allow_alipay']    = 1;
            $insertData['pay_price']       = $pay_price;
            $insertData['order_status']    = 1;
            $insertData['add_time']        = TIMESTAMP;

...
function tomgetfileurl($url){
    ...
    if(!empty($url)){
        if(!preg_match('/^(http|https):/', $url) ){
            $pic_url = (preg_match('/^(http|https):/', $_G['setting']['attachurl']) ? '' : $_G['siteurl']).$_G['setting']['attachurl'].'common/'.$url;
        }else{
            $pic_url = $url;
        }
    }
...
function tomcreateFile($config = array()){
    ...
    if(!empty($options['value'])){
        if(!preg_match('/^(http|https):/', $options['value']) ){
            $pic_url = (preg_match('/^(http|https):/', $_G['setting']['attachurl']) ? '' : $_G['siteurl']).$_G['setting']['attachurl'].'tomwx/'.$options['value'];
        }else{

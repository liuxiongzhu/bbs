...
function tomgetfileurl($url){
    ...
    if(!empty($url)){
        if(!preg_match('/^http/', $url) ){
            if(strpos($value['picurl'], 'source/plugin/tom_tchongbao/') === FALSE){
                $pic_url = (preg_match('/^http/', $_G['setting']['attachurl']) ? '' : $_G['siteurl']).$_G['setting']['attachurl'].'tomwx/'.$url;
            }else{
...
function tomcreateFile($config = array()){
    ...
    if(!empty($options['value'])){
        if(!preg_match('/^http/', $options['value']) ){
            if(strpos($options['value'], 'source/plugin/tom_tchongbao/') === FALSE){

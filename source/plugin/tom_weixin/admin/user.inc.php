...
$moduleBaseUrl = $adminBaseUrl.'&tmod=user';
$moduleListUrl = $adminListUrl.'&tmod=user';
$moduleFromUrl = $adminFromUrl.'&tmod=user';
...
if ($formhash == FORMHASH && $act == 'unbind'){
    ...
    cpmsg($tomScriptLang['unbind_succ'], $moduleListUrl, 'succeed');
}else{
    $pagesize = 15;
	...
	foreach ($userList as $key => $value){
        ...
        echo '<td>' . $bindTime . '</td>';
        echo '<td>';
        echo '<a href="'.$moduleBaseUrl.'&act=unbind&openid='.$value['openid'].'&formhash='.FORMHASH.'">' . $tomScriptLang['unbind'] . '</a>';
        echo '</td>';
        echo '</tr>';
	}
	showtablefooter();

...
$moduleBaseUrl = $adminBaseUrl.'&tmod=module';
$moduleListUrl = $adminListUrl.'&tmod=module';
$moduleFromUrl = $adminFromUrl.'&tmod=module';
...
if($act == 'config'){
    ...
    showformheader($moduleFromUrl.'&act=save&moduleid='.$moduleInfo['module_id']);
    showtableheader();
    ...
    if(!$checkCmd){
        cpmsg($sysCmdString.$tomScriptLang['not_cmd_set'], $moduleListUrl.'&act=config&moduleid='.$moduleInfo['module_id'], 'succeed');
    }else{
    ...
    foreach ($allModuleList as $key => $moduleid) {
        ...
        if($isInstall){
            ...
            if(isset($moduleConfig['admin']) && $moduleConfig['admin'] == 1){
                echo '&nbsp;|&nbsp;<a href="'.$moduleBaseUrl.'&act=admin&moduleid='.$moduleid.'"><font color="#266f02">' . $moduleConfig['admin_name'] . '</font></a>';
            }
    ...
    showtableheader();
    echo '<tr><th colspan="15" class="partition">' . $tomScriptLang['old_admin_menu'] . '</th></tr>';
    echo '<tr><th colspan="15">';
    echo '&nbsp;&nbsp;<a class="addtr" href="'.$adminBaseUrl.'&tmod=plugin">' . $tomScriptLang['plugin_list_title'] . '</a>';
    echo '&nbsp;&nbsp;<a class="addtr" href="'.$adminBaseUrl.'&tmod=user">' . $tomScriptLang['user_list_title'] . '</a>';
    echo '</th></tr>';
    showtablefooter();
}

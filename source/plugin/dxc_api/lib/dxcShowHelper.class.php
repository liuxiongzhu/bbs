...
class dxcShowHelper{
	...
	static public function radio($args, $info = array()){
		...
		if($int_val && !isset($info[$name])) $info[$name] = $int_val;
	...
	static public function dateline($args, $info){
		...
		$min_dateline = $_G['timestamp'] - 20*360*24*3600;
		...
		return '<script src="static/js/calendar.js?3R4" type="text/javascript" type="text/javascript"></script><input name="set['.$name_start.']" type="text" onclick="showcalendar(event, this, '.$date_type.')" value="'.$start_time.'" class="px length_'.$length.' '.$date_type.' mr20"><span class="mr20"> '.stlang('_to').' </span><input name="set['.$name_end.']" type="text" value="'.$end_time.'" onclick="showcalendar(event, this, '.$date_type.')" class="px length_'.$length.' '.$date_type.' mr20">';
	...
	static public function show_num_range($num){
		...
		if($num < 4){
			return '<em class="red" title="'.$title.'">'.$num.'</em>';
		}else if($num > 3){
			return '<em title="'.$title.'">'.$num.'</em>';
		}
	}

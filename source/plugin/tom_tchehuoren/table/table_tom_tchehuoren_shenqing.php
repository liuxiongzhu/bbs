...
class table_tom_tchehuoren_shenqing extends discuz_table{
	public function __construct() {
        parent::__construct();
		$this->_table = 'tom_tchehuoren_shenqing';
		$this->_pk    = 'id';
	}
    ...
    public function fetch_by_user_id($id,$field='*') {
		return DB::fetch_first("SELECT $field FROM %t WHERE user_id=%d ORDER BY id DESC", array($this->_table, $id));
	}
    
    public function fetch_by_order_no($order_no,$field='*') {
		return DB::fetch_first("SELECT $field FROM %t WHERE order_no=%s ORDER BY id DESC", array($this->_table, $order_no));
	}
	...
	public function delete_by_user_id($id) {
		return DB::query("DELETE FROM %t WHERE user_id=%d", array($this->_table, $id));
	}

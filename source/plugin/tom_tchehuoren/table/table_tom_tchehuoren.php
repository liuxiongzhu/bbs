...
class table_tom_tchehuoren extends discuz_table{
	public function __construct() {
        parent::__construct();
		$this->_table = 'tom_tchehuoren';
		$this->_pk    = 'id';
	}
    ...
    public function fetch_by_user_id($user_id,$field='*') {
		return DB::fetch_first("SELECT $field FROM %t WHERE user_id=%d ORDER BY id DESC", array($this->_table, $user_id));
	}
    
    public function fetch_by_openid($openid,$field='*') {
		return DB::fetch_first("SELECT $field FROM %t WHERE openid=%s ORDER BY id DESC", array($this->_table, $openid));
	}

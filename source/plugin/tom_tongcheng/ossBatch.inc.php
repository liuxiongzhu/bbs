...
define("TOM_OSS_ACCESS_ID", $alioss_access_id);  
define("TOM_OSS_ACCESS_KEY", $alioss_access_key); 
define("TOM_OSS_ENDPOINT", $alioss_endpoint); 
define("TOM_OSS_BUCKET", $alioss_buckey); 

include DISCUZ_ROOT.'./source/plugin/tom_tongcheng/class/OSS/load.php';

$whereStr   = " AND oss_status=0 ";
$photoList  = C::t('#tom_tongcheng#tom_tongcheng_photo')->fetch_all_list(" {$whereStr} "," ORDER BY id DESC ",0,10);
$overStatus = 0;
$errorStatus = 0;
$okStatus = 0;
        ...
        if(!preg_match('/^http/', $value['picurl']) ){
            ...
            if(file_exists($filepicurl)){
                ...
                if(!empty($r['picurl'])){
                    ...
                    C::t('#tom_tongcheng#tom_tongcheng_photo')->update($value['id'],$updateData);
                }else{
...
if($okStatus == 1){
    $page++;
    $ossBatchUrl = $_G['siteurl']."plugin.php?id=tom_tongcheng:ossBatch";
    $ch = curl_init();

...
if($act == "fabu" && $_GET['formhash'] == FORMHASH && $userStatus == true){
    ...
    if($typeInfo['free_status'] == 2 && $typeInfo['fabu_price'] > 0 && $userInfo['editor']==0){
        ...
        if($tongchengConfig['pay_score_yuan'] > 0 && $fabuPayStatus == 1){
    ...
    if(C::t('#tom_tongcheng#tom_tongcheng')->insert($insertData)){
        ...
        if($modelInfo['is_sfc'] == 1 && !empty($typeInfo['sfc_chufa_attr_id']) && !empty($typeInfo['sfc_mude_attr_id'])  && !empty($typeInfo['sfc_time_attr_id']) && !empty($typeInfo['sfc_renshu_attr_id'])){

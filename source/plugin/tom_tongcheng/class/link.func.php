...
function tomreplaceMap(){
    ...
    if(isset($tom_link_rule['tom_tc114'])){
        $r['code']      = 1;
        $r['search'][]  = 'plugin.php?id=tom_tc114';
        $r['replace'][] = $tom_link_rule['tom_tc114']['rk']."?id=".$tom_link_rule['tom_tc114']['bs'];
    }
    if(isset($tom_link_rule['tom_tcyikatong'])){
        $r['code']      = 1;
        $r['search'][]  = 'plugin.php?id=tom_tcyikatong';
        $r['replace'][] = $tom_link_rule['tom_tcyikatong']['rk']."?id=".$tom_link_rule['tom_tcyikatong']['bs'];
    }
...
function tomoutput(){
    ...
    if(file_exists(DISCUZ_ROOT."./source/plugin/tom_link/data/rule.php")){
        ...
        if($r['code'] == 1){
            ...
            echo $content;
            echo '<script src="source/plugin/tom_tongcheng/images/miniprogram.js?v=201805031"></script>';
        }else{
            $no_link_rule = 1;
        }
    }else{

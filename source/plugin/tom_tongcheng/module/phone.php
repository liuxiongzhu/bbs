...
if($act == 'save' && $_GET['formhash'] == FORMHASH){
    ...
    $must       = intval($_GET['must'])>0? intval($_GET['must']):0;
    ...
    if($ucenterfilenameExists && !empty($ucenterConfig['login_type']) && $must == 0){
    ...
    $userInfoTmpTel = C::t('#tom_tongcheng#tom_tongcheng_user')->fetch_by_tel($tel);
    if($userInfoTmpTel && $userInfoTmpTel['id'] != $__UserInfo['id'] && $must == 1){
        $updateData = array();
        $updateData['tel']          = '';
    ...
    if(!empty($pwd) && $showPwdInput == 1){
        $memberInfoTmpTel = C::t('#tom_ucenter#tom_ucenter_member')->fetch_by_tel($tel);
        if($memberInfoTmpTel && $memberInfoTmpTel['uid'] != $__UserInfo['member_id'] && $must == 1){
            $updateData = array();
            $updateData['tel']          = '';
            C::t('#tom_ucenter#tom_ucenter_member')->update($memberInfoTmpTel['uid'],$updateData);
        }
...
$saveMustUrl = "plugin.php?id=tom_tongcheng&site={$site_id}&mod=phone&act=save&must=1&formhash=".FORMHASH;

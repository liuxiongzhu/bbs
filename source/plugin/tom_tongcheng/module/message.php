...
if($act == 'create' && $_GET['formhash'] == FORMHASH){
    ...
    if($to_user_id == $__UserInfo['id']){
        tomheader('location:'.$_G['siteurl']."plugin.php?id=tom_tongcheng&site={$site_id}&mod=message");exit;
...
//        C::t('#tom_tongcheng#tom_tongcheng_pm_message')->insert($insertData);
    ...
    tomheader('location:'.$_G['siteurl']."plugin.php?id=tom_tongcheng&site={$site_id}&mod=message&act=sms&pm_lists_id=".$pm_lists_id);exit;
    ...
    $showMustPhoneBtn = 0;
    if(array_search('4',$must_phone_projectArr) !== false && empty($__UserInfo['tel']) && $__UserInfo['editor']==0){
        $showMustPhoneBtn = 1;

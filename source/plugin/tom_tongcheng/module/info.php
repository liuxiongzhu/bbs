...
$showDingwei = 0;
if($modelInfo['open_dingwei'] == 1 && $tongchengInfo['is_dingwei'] == 1){
    $showDingwei = 1;
    $juli = 0;
...
$showBuyTelBtn = 0;
if($typeInfo['open_tel_price'] == 1){
    if($__UserInfo['id'] > 0){
        $orderListTmp = C::t('#tom_tongcheng#tom_tongcheng_order')->fetch_all_list(" AND tongcheng_id={$tongchengInfo['id']} AND user_id={$__UserInfo['id']} AND order_type=9 AND order_status=2 ","ORDER BY id DESC",0,1);
        if(is_array($orderListTmp) && !empty($orderListTmp)){
        }else if($__UserInfo['editor'] == 1){
        }else{
            $showBuyTelBtn = 1;

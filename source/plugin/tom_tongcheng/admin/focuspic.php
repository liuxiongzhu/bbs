...
if($_GET['act'] == 'add'){
    ...
    echo '<th>' . $Lang['focuspic_type_id'] . '</th>';
    ...
    foreach ($focuspicList as $key => $value) {
        ...
        if($value['type_id'] == 2){
            if($modelInfoTmp){
                echo '<td>'.$modelInfoTmp['name'].'>>'. $Lang['focuspic_type_id_2'] . '</td>';
            }else{
                echo '<td>' . $Lang['focuspic_type_id_2'] . '</td>';
            }
        }else if($value['type_id'] == 3){
            if($modelInfoTmp){
                echo '<td>'.$modelInfoTmp['name'].'>>'. $Lang['focuspic_type_id_3'] . '</td>';
            }else{
                echo '<td>' . $Lang['focuspic_type_id_3'] . '</td>';
            }
        }else{
            echo '<td>' . $Lang['focuspic_type_id_1'] . '</td>';
        }
        if($value['site_id'] > 1){
...
function __create_info_html($infoArr = array()){
    ...
    echo $sitesStr;
    $type_id_item = array(1=>$Lang['focuspic_type_id_1'],2=>$Lang['focuspic_type_id_2'],3=>$Lang['focuspic_type_id_3']);
    tomshowsetting(true,array('title'=>$Lang['focuspic_type_id'],'name'=>'type_id','value'=>$options['type_id'],'msg'=>$Lang['focuspic_type_id_msg'],'item'=>$type_id_item),"radio");
    
    $modelList = C::t('#tom_tongcheng#tom_tongcheng_model')->fetch_all_list(" "," ORDER BY id DESC ",0,100);
    $modelStr = '<tr class="header"><th>'.$Lang['focuspic_model_id'].'</th><th></th></tr>';
    $modelStr.= '<tr><td width="300"><select style="width: 260px;" name="model_id" id="model_id">';
    $modelStr.=  '<option value="0">'.$Lang['focuspic_model_id_0'].'</option>';

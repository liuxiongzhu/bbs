...
SCRIPT;
    ...
    $photoListTmp = C::t('#tom_tongcheng#tom_tongcheng_photo')->fetch_all_list(" AND tongcheng_id={$_GET['id']} "," ORDER BY id ASC ",0,20);
    if(is_array($photoListTmp) && !empty($photoListTmp)){
        foreach ($photoListTmp as $key => $value){
            if(!preg_match('/^http/', $value['picurl']) ){
                if(strpos($value['picurl'], 'source/plugin/tom_tongcheng/data/') === false){    
...
EOF;
    ...
    $modBasePageUrl = $modBaseUrl."&model_id={$model_id}&type_id={$type_id}&cate_id={$cate_id}&user_id={$user_id}&site_id={$site_id}&shenhe_status={$shenhe_status}";
    ...
    $sitesStr = '<tr><td width="100" align="right"><b>'.$Lang['index_shenhe_status'].'</b></td>';
    $sitesStr.= '<td><select style="width: 260px;" name="shenhe_status" id="shenhe_status">';
    $sitesStr.=  '<option value="0">'.$Lang['index_shenhe_status'].'</option>';
    $sitesStr.=  '<option value="1" '.$shenhe_status_1.' >'.$Lang['index_shenhe_status_1'].'</option>';
    $sitesStr.=  '<option value="2" '.$shenhe_status_2.' >'.$Lang['index_shenhe_status_2'].'</option>';
    $sitesStr.=  '<option value="3" '.$shenhe_status_3.' >'.$Lang['index_shenhe_status_3'].'</option>';
    $sitesStr.= '</select></td></tr>';
    echo $sitesStr;
    ...
    foreach ($tongchengList as $key => $value) {
        ...
        if($value['over_days'] > 0 ){
            echo '<li>';
            echo '<b>'.$Lang['index_over_days'].'&nbsp;:&nbsp;</b><font color="#0a9409">'.$value['over_days'] . $Lang['index_over_days_tian'] . '</font>&nbsp;&nbsp;';
            if($value['over_time'] > 0){
                echo '<font color="#f00">(' . dgmdate($value['over_time'],"Y-m-d",$tomSysOffset) . '&nbsp;' . $Lang['index_over_days_guoqi'] . ')</font>';
            }else{
                echo '<font color="#f00">(--)</font>';
            }
            echo '</li>';
        }

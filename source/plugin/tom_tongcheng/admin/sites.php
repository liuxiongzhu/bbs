...
if($_GET['act'] == 'add'){
    ...
    if(file_exists(DISCUZ_ROOT.'./source/plugin/tom_tcadmin/tom_tcadmin.inc.php')){
        ...
        echo '<tr><td  class="tipsblock" s="1"><ul id="tipslis">';
        echo '<li><font color="#fd0d0d">' . $Lang['sites_help_0'] . '</font></li>';
        echo '<li>' . $Lang['sites_help_1'] . '</li>';
        echo '</ul></td></tr>';
        showtablefooter();
    }
    __create_nav_html();
    ...
    echo '<th>' . $Lang['sites_cate_id'] . '</th>';
    echo '<th>' . $Lang['sites_is_hot'] . '</th>';
    echo '<th>' . $Lang['sites_open_sites'] . '</th>';
    echo '<th>' . $Lang['sites_site_one'] . '</th>';
    echo '<th>' . $Lang['sites_status'] . '</th>';
    echo '<th>' . $Lang['sites_paixu'] . '</th>';
    ...
    foreach ($sitesList as $key => $value) {
        ...
        $sites_cateInfo = C::t('#tom_tongcheng#tom_tongcheng_sites_cate')->fetch_by_id($value['cate_id']);
        ...
        if($value['cate_id'] > 0){
            echo '<td>' . $sites_cateInfo['name'] . '</td>'; 
        }else{
            echo '<td><font color="#8e8e8e">' . $Lang['sites_cate_id_0'] . '</font></td>';
        }
        if($value['is_hot'] == 1){
            echo '<td><font color="#238206">' . $Lang['sites_is_hot_1'] . '</font></td>';
        }else{
            echo '<td><font color="#fd0d0d">' . $Lang['sites_is_hot_0'] . '</font></td>';
        }
        if($value['open_sites'] == 1){
            echo '<td><font color="#238206">' . $Lang['sites_open_sites_1'] . '</font></td>';
        }else{
            echo '<td><font color="#fd0d0d">' . $Lang['sites_open_sites_0'] . '</font></td>';
        }
        if($value['site_one'] == 1){
...
function __get_post_data($infoArr = array()){
    ...
    $open_sites         = isset($_GET['open_sites'])? intval($_GET['open_sites']):0;
    $cate_id            = isset($_GET['cate_id'])? intval($_GET['cate_id']):0;
    $is_hot             = isset($_GET['is_hot'])? intval($_GET['is_hot']):0;
    ...
    $mall_fc_scale      = isset($_GET['mall_fc_scale'])? intval($_GET['mall_fc_scale']):0;
    $tt_fc_scale        = isset($_GET['tt_fc_scale'])? intval($_GET['tt_fc_scale']):0;
    $hehuoren_fc_open   = isset($_GET['hehuoren_fc_open'])? intval($_GET['hehuoren_fc_open']):2;
    
    $ruzhu_price            = isset($_GET['ruzhu_price'])? addslashes($_GET['ruzhu_price']):'';
    $vip_ruzhu_price        = isset($_GET['vip_ruzhu_price'])? addslashes($_GET['vip_ruzhu_price']):'';
    $vip_ruzhu_price_two    = isset($_GET['vip_ruzhu_price_two'])? addslashes($_GET['vip_ruzhu_price_two']):'';
    
    $index_color_menu    = isset($_GET['index_color_menu'])? addslashes($_GET['index_color_menu']):'';
    $index_white_menu    = isset($_GET['index_white_menu'])? addslashes($_GET['index_white_menu']):'';
    ...
    $data['mall_fc_scale']      = $mall_fc_scale;
    $data['kj_fc_scale']        = $kj_fc_scale;
    $data['114_fc_scale']       = $_114_fc_scale;
    $data['vip_fc_scale']       = $vip_fc_scale;
    $data['tt_fc_scale']        = $tt_fc_scale;
    $data['dingyue_qrcode']     = $dingyue_qrcode;
    $data['hehuoren_fc_open']   = $hehuoren_fc_open;
    
    $data['ruzhu_price']            = $ruzhu_price;
    $data['vip_ruzhu_price']        = $vip_ruzhu_price;
    $data['vip_ruzhu_price_two']    = $vip_ruzhu_price_two;
    
    $data['index_color_menu']    = $index_color_menu;
    $data['index_white_menu']    = $index_white_menu;
...
function __create_info_html($infoArr = array()){
    ...
    $options = array(
        ...
        'hehuoren_fc_open'          => 2,
        'ruzhu_price'               => '0.00',
        'vip_ruzhu_price'           => '0.00',
        'vip_ruzhu_price_two'       => '0.00',
        'index_color_menu'          => '',
        'index_white_menu'          => '',
        'paixu'                     => 1000,
    );
    ...
    $open_sites_item = array(0=>$Lang['sites_open_sites_0'],1=>$Lang['sites_open_sites_1']);
    tomshowsetting(true,array('title'=>$Lang['sites_open_sites'],'name'=>'open_sites','value'=>$options['open_sites'],'msg'=>$Lang['sites_open_sites_msg'],'item'=>$open_sites_item),"radio");
    
    $sites_cateList = C::t('#tom_tongcheng#tom_tongcheng_sites_cate')->fetch_all_list(" "," ORDER BY paixu ASC,id DESC ",0,100);
    $cateStr = '<tr class="header"><th>'.$Lang['sites_cate_id'].'</th><th></th></tr>';
    $cateStr.= '<tr><td width="300"><select style="width: 260px;" name="cate_id" id="cate_id">';
    $cateStr.=  '<option value="0">'.$Lang['sites_cate_id_0'].'</option>';
    ...
    $cateStr.= '</select></td><td>'.$Lang['sites_cate_id_msg'].'</td></tr>';
    echo $cateStr;
    
    $is_hot_item = array(0=>$Lang['sites_is_hot_0'],1=>$Lang['sites_is_hot_1']);
    tomshowsetting(true,array('title'=>$Lang['sites_is_hot'],'name'=>'is_hot','value'=>$options['is_hot'],'msg'=>$Lang['sites_is_hot_msg'],'item'=>$is_hot_item),"radio");
    ...
    tomshowsetting(true,array('title'=>$Lang['sites_114_fc_scale'],'name'=>'114_fc_scale','value'=>$options['114_fc_scale'],'msg'=>'<font color="#fd0d0d">%</font>&nbsp;&nbsp;'.$Lang['sites_114_fc_scale_msg']),"input");
    tomshowsetting(true,array('title'=>$Lang['sites_vip_fc_scale'],'name'=>'vip_fc_scale','value'=>$options['vip_fc_scale'],'msg'=>'<font color="#fd0d0d">%</font>&nbsp;&nbsp;'.$Lang['sites_vip_fc_scale_msg']),"input");
    tomshowsetting(true,array('title'=>$Lang['sites_mall_fc_scale'],'name'=>'mall_fc_scale','value'=>$options['mall_fc_scale'],'msg'=>'<font color="#fd0d0d">%</font>&nbsp;&nbsp;'.$Lang['sites_mall_fc_scale_msg']),"input");
    tomshowsetting(true,array('title'=>$Lang['sites_tt_fc_scale'],'name'=>'tt_fc_scale','value'=>$options['tt_fc_scale'],'msg'=>'<font color="#fd0d0d">%</font>&nbsp;&nbsp;'.$Lang['sites_tt_fc_scale_msg']),"input");
    ...
    tomshowsetting(true,array('title'=>$Lang['sites_ruzhu_price'],'name'=>'ruzhu_price','value'=>$options['ruzhu_price'],'msg'=>$Lang['sites_ruzhu_price_msg']),"input");
    tomshowsetting(true,array('title'=>$Lang['sites_vip_ruzhu_price'],'name'=>'vip_ruzhu_price','value'=>$options['vip_ruzhu_price'],'msg'=>$Lang['sites_vip_ruzhu_price_msg']),"input");
    tomshowsetting(true,array('title'=>$Lang['sites_vip_ruzhu_price_two'],'name'=>'vip_ruzhu_price_two','value'=>$options['vip_ruzhu_price_two'],'msg'=>$Lang['sites_vip_ruzhu_price_two_msg']),"input");
    
    tomshowsetting(true,array('title'=>$Lang['sites_paixu'],'name'=>'paixu','value'=>$options['paixu'],'msg'=>$Lang['sites_paixu_msg']),"input");
    
    $menuStr = '<tr class="header"><th>'.$Lang['sites_index_color_menu'].'</th><th></th></tr>';
    $menuStr.= '<tr><td colspan="2"><textarea rows="6" name="index_color_menu" cols="100" >'.$options['index_color_menu'].'</textarea>&nbsp;&nbsp;'.$Lang['sites_index_color_menu_msg'].'</td></tr>';
    $menuStr.= '<tr class="header"><th>'.$Lang['sites_index_white_menu'].'</th><th></th></tr>';
    $menuStr.= '<tr><td colspan="2"><textarea rows="6" name="index_white_menu" cols="100" >'.$options['index_white_menu'].'</textarea>&nbsp;&nbsp;'.$Lang['sites_index_white_menu_msg'].'</td></tr>';
    echo $menuStr;
...
function __create_nav_html($infoArr = array()){
    ...
    if($_GET['act'] == 'add'){
        ...
        tomshownavli($Lang['sites_add'],$modBaseUrl."&act=add",false);
        tomshownavli($Lang['sites_cate_list_title'],$adminBaseUrl.'&tmod=sitesCate',false);
    }
    tomshownavfooter();
}

...
function __get_post_data($infoArr = array()){
    ...
    $forbid_wordStr = str_replace("\r\n","{n}",$forbid_word); 
    $forbid_wordStr = str_replace("\n","{n}",$forbid_wordStr);
    $forbid_wordList = explode("{n}", $forbid_wordStr);
    
    $forbid_word_list_tmp = array();
    if(is_array($forbid_wordList) && !empty($forbid_wordList)){
        foreach ($forbid_wordList as $key => $value){
            $value = trim($value);
            if(!empty($value)){
                $forbid_word_list_tmp[] = $value;
            }
        }
    }
    
    $forbid_word_ok = '';
    if(!empty($forbid_word_list_tmp)){
        $forbid_word_ok = implode("\r\n", $forbid_word_list_tmp);
    }

    $data['xieyi_txt']    = $xieyi_txt;
    $data['forbid_word']  = $forbid_word_ok;

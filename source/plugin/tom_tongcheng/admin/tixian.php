...
if($_GET['act'] == 'ok1'){
    ...
    foreach ($tixianList as $key => $value) {
        ...
        echo '<td>' .$userInfo['nickname'] . '(ID:'.$userInfo['id'].')'.'(<a target="_blank" href="'.$adminBaseUrl.'&tmod=user'.'&act=moneylog&user_id='.$userInfo['id'].'&formhash='.FORMHASH.'">' . $Lang['moneylog_list_title']. '</a>)</td>';

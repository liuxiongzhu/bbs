<?php

if(!defined('IN_DISCUZ')) {
	exit('Access Denied');
} 

class table_tom_weixin_vote_item extends discuz_table{
	public function __construct() {
        parent::__construct();
		$this->_table = 'tom_weixin_vote_item';
		$this->_pk    = 'id';
	}

    public function fetch_by_id($id,$field='*') {
		return DB::fetch_first("SELECT $field FROM %t WHERE id=%d", array($this->_table, $id));
	}
    
    public function insert_id() {
		return DB::insert_id();
	}
    
    public function fetch_by_vid_tel($vid,$tel) {
		return DB::fetch_first("SELECT * FROM %t WHERE vote_id=%d AND tel=%s ", array($this->_table, $vid,$tel));
	}
    
    public function fetch_all_list($condition,$orders = '',$start = 0,$limit = 10) {
		$data = DB::fetch_all("SELECT * FROM %t WHERE 1 %i $orders LIMIT $start,$limit",array($this->_table,$condition));
		return $data;
	}
    
    public function fetch_all_like_list($condition,$name,$orders = '',$start = 0,$limit = 10) {
		$data = DB::fetch_all("SELECT * FROM %t WHERE 1 %i AND name LIKE %s $orders LIMIT $start,$limit",array($this->_table,$condition,'%'.$name.'%'));
		return $data;
	}
    
    public function fetch_all_count($condition) {
        $return = DB::fetch_first("SELECT count(*) AS num FROM ".DB::table($this->_table)." WHERE 1 $condition ");
		return $return['num'];
	}
    
    public function fetch_all_like_count($condition,$name) {
        $return = DB::fetch_first("SELECT count(*) AS num FROM ".DB::table($this->_table)." WHERE 1 $condition AND name LIKE %s ",array('%'.$name.'%'));
		return $return['num'];
	}
    
    public function fetch_all_sun($condition) {
        $return = DB::fetch_first("SELECT SUM(num) AS allnum FROM ".DB::table($this->_table)." WHERE 1 $condition ");
		return $return['allnum'];
	}
	
	public function delete_by_id($id) {
		return DB::query("DELETE FROM %t WHERE id=%d", array($this->_table, $id));
	}
    
    public function delete_by_vote_id($vote_id) {
		return DB::query("DELETE FROM %t WHERE vote_id=%d", array($this->_table, $vote_id));
	}

}

